...
public class BottomListDialog {
    ...
    public static void showSimpleDialog(Context ctx, final OnItemClickListener listener, String[]... groups) {
        ...
        showDialog(ctx, R.layout.dialog_bottom_container, R.layout.dialog_bottom_group, R.layout.dialog_bottom_item, R.id.text, 1, 16, transColor,transColor , listener, groups);
    }
    ...
    public static void showSimpleDialog(Context ctx, final OnItemClickListener listener,int itemLayout, int itemTextId, String[]... groups) {
        int transColor = ctx.getResources().getColor(android.R.color.transparent);
        showDialog(ctx, R.layout.dialog_bottom_container, R.layout.dialog_bottom_group, itemLayout, itemTextId, 1, 16, transColor,transColor , listener, groups);
    }
    ...
    public static void showDialog(Context ctx, int containerLayout, int groupLayout, int itemLayout, int itemTextId, int groupDivderHeight,
            int itemDividerHeight, int itemDividerColor, int groupDividerColor, final OnItemClickListener listener, String[]... groups) {
        // final BottomSheetDialog dialog = new BottomSheetDialog(ctx, R.style.CustomBottomSheetDialogTheme);
        final BottomSheetDialog dialog = new BottomSheetDialog(ctx); // 背景透明，使用代码hack的方式
        ...
        for (int i = 0; i < groups.length; i++) {
            ...
            for (int k = 0; k < groups[i].length; k++) {
                ...
                itemDivider.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, oneDP * groupDivderHeight));
            ...
            groupDivider.setLayoutParams(new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, itemDividerHeight * oneDP));
